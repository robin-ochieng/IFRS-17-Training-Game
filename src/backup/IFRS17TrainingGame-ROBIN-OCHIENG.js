  ...
  const loadGuestProgressData = () => {
    try {
      ...
      if (savedState) {
        ...
        const restoredQuestion = savedState.currentQuestion ?? 0;
        ...
        const restoredUnlocked = Array.isArray(savedState.unlockedModules)
          ? savedState.unlockedModules.filter((m) => Number.isInteger(m) && m >= 0)
          : [];
        ...
        // Compute unlocked modules to ensure resume target stays accessible
        const impliedUnlockMax = Math.max(restoredModule, restoredQuestion >= 0 ? restoredModule : 0, ...restoredCompleted.map((m) => m + 1));
  ...
  const saveProgress = async () => {
    ...
    try {
      if (isGuest) {
        ...
        if (success) {
          console.log('✅ Progress saved locally and queued for Supabase');
        } else {
          console.warn('⚠️ Failed to persist progress locally; will retry on next save');
        }
      }
    } catch (error) {
